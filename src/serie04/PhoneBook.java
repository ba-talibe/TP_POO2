...
public interface PhoneBook<C extends Contact & Comparable<C>, N> {
     ...
     *     forall i, 0 <= i < nums.size() : nums.get(i) != null </pre>
     * @post <pre>
     *     Let list ::= phoneNumbers(p)
     *     contains(p)
     *     list.size() <= nums.size()
     *     forall i, 0 <= i < nums.size(), exist j, 0 <= j < list.size() :
     *         j <= i
     *         list.get(j).equals(nums.get(i))
     *         forall k, 0 <= k < list.size() :
     ...
     *                 phoneNumbers(p).get(i).equals(old phoneNumbers(p).get(i))
     *     old phoneNumbers(p).contains(n)
